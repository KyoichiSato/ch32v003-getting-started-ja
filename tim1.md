...
2. Tdelayは比較キャプチャレジスタ (CH1CVR) の値によって決定され、Tpulseはオートリロード値レジスタ ATRLR および比較キャプチャレジスタ (ATRLR - CH1CVR) の値によって決定される。
...
エンコーダモードで周期更新イベントが発生するのは避けたいので、リロード値レジスタに 0xFFFFを設定し、タイマカウンタを 0x8000などに設定する。

エンコーダモードでは、タイマ内部の比較キャプチャレジスタ、プリスケーラ、リピートカウントレジスタなどは通常通り動作する。
...
```
エンコーダモードの信号は [エンコーダモード](#エンコーダモード)の説明を参照
