...
#### 用語の整理
* タイマカウンタの1周期の完了を示す update event (UV, UEV) は、「アップデートイベント」だと何のアップデートなのか伝わらないので、この文章内では「周期完了イベント」と書く。
...
これらのクロックソースは、TRGOとして他のタイマやADCなどの周辺機器へ出力することも可能である。

#### カウンタ CNT
タイマのコアは 16-bitカウンタ (CNT) であり、入力されたクロック CK_PSCはプリスケーラ (PSC) によって分周され、CK_CNTとなって CNTへ出力される。
...
4. コンペア/キャプチャチャネルピン: TIMx_CHx → TIx → TIxFPx
5. 他のタイマからの入力: ITRx
...
4. 外部クロック入力ピンからの信号 ETRF

#### 外部クロックソースモード2
ECEビットをセットすると、外部クロックモード2 となる。ETRFが CK_PSCとして選択され外部クロック入力ピン信号のカウントを行う。
...
タイマの中核はコンペア/キャプチャレジスタであり、これに対して入力部にはデジタルフィルタ、周波数分周、チャネル間マルチプレクスが存在し、出力部にはコンパレータおよび出力制御が存在する。
...
### 機能及び実装
#### インプットキャプチャモード
インプットキャプチャモードはタイマの基本機能の1つである。

インプットキャプチャモードの原理は、ICxPS信号の所定エッジが検出されたときにキャプチャイベントが発生し、現在のカウンタ値がコンペア/キャプチャレジスタ (R16_TIMx_CHCTLRx) にラッチされることである。

キャプチャイベントが発生すると、R16_TIMx_INTFR内の CCxIFがセットされ、有効に設定されていれば、対応する割込みまたは DMAも生成される。
...
6. 必要に応じて CC1IEおよび CC1DEビットを設定し、割込みまたは DMAの有効化を構成する。これにより比較キャプチャチャネルの構成が完了する。
...
リファレンスマニュアルを読んでみて、CH32V003のマニュアルは書いていないことが多すぎると感じたけれど、CH32V003のペリフェラルの元になっている STM32のリファレンスマニュアルの説明がそもそも不十分だった。
