...
xには A/C/Dのポート名が入る GPIOA->CFGR とか GPIOC->CFGR とか GPIOD->CFGR。
...
[15: 8]          RO Reserved
[ 7: 0] LCK[7:0] RW 1を書くと対応するポートの CFGR構成をロックする。
                    LCKKでロックする前のみ変更可能。
LOCKシーケンスが実行されると、次回リセットまでポートの構成を変更できない。
```
CFGRで入出力の設定、INDRが入力レジスタ、OUTDRが出力レジスタ、BSHRがビットセット／リセットレジスタ。これだけで使えるので簡単。
